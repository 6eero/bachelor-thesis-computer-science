...
Un utente, per poter utilizzare la dashboard, deve avere un account appositamente creato da un utente con ruolo ADMIN. L'amministratore ha il compito di gestire l'accesso e i privilegi degli utenti all'interno del sistema. La creazione di un nuovo account per un utente avviene attraverso la dasboard di Cognito tramite una procedura che implica la registrazione delle informazioni essenziali, come nome ed email. Il sistema AWS Cognito genera automaticamente una password casuale e invia un'email all'indirizzo fornito, contenente la password temporanea e un link. Il link rimanda a una pagina dove il nuovo utente deve inserire la password monouso ricevuta e scegliere una nuova password, che dovrà essere confermata inserendola due volte. Una volta creato e attivato l'account, l'utente può accedere alla dashboard inserendo le proprie credenziali o cambiare la sua password, nela caso la dimenticasse.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{img/diagrammi/use_case_login_e_manage.pdf} 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{img/diagrammi/uml_menufields.pdf} 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{img/diagrammi/use_case_manage_projects.pdf} 
